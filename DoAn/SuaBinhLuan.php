...
<body>
    <header>
        <div class="logo">
            <ul>
                <li><a href="QuanLyDanhMuc.php" style="font-size: 2vw;">Sửa Bình Luận</a></li>
            </ul>
    ...
    <main>
        ...
        $id = $_GET['id'];
        $query = "SELECT * from binhluan WHERE MaBinhLuan='$id'";
        $result = $conn->query($query);
        $ds = $result->fetch_assoc();
        ?>

        <table>
            <form method="POST" action="Sua.php?hd=suabl&id=<?php echo $ds['MaBinhLuan'] ?>">
                
            <tr>
                    <td><label for="noidung">Nội Dung : </label></td>
                    <td><textarea id="noidung" name="noidung" rows="2" cols="52" placeholder="<?php echo $ds['NoiDung'] ?>"></textarea></td>

                </tr>
                <tr>
                    <td><label for="ngaytao">Ngày Tạo:</label></td>
                    <td><input type="date" id="ngaytao" value="<?php echo $ds['NgayTao']?>" size="50" name="ngaytao"></td>
                </tr>
                <tr>
