...
if ($_SESSION['dn'] == false) {
    ...
    <body>
        ...
        <main>
            ...
            <?php
                $search1 = "SELECT *  FROM hoadon,chitiethoadon,sanpham,hinhanh WHERE hinhanh.MaSanPham=sanpham.MaSanPham AND sanpham.MaSanPham=chitiethoadon.MaSanPham AND hoadon.MaHoaDon=chitiethoadon.MaHoaDon AND MaKhachHang='{$_SESSION['makh']}' AND hoadon.MaHoaDon='{$_GET['id']}' ";
                $giatri1 = $conn->query($search1);
                $search2 = "SELECT TenNguoiNhan,DiaChiNhanHang,NgayLap,SUM(SoLuong) as SL ,DonGia  FROM hoadon,chitiethoadon,sanpham,hinhanh WHERE hinhanh.MaSanPham=sanpham.MaSanPham AND sanpham.MaSanPham=chitiethoadon.MaSanPham AND hoadon.MaHoaDon=chitiethoadon.MaHoaDon AND MaKhachHang='{$_SESSION['makh']}' AND hoadon.MaHoaDon='{$_GET['id']}' group by sanpham.MaSanPham ";
                $giatri2 = $conn->query($search2);
                $ds = $giatri1->fetch_assoc();
                ?>
                <div style="display: flex;padding-top:10px">
                <p style="font-weight: bold;margin-left:20px;font-size:18px;">Chi tiết đơn hàng</p>
                <p style="margin-left:700px;color:blue"><?php echo $ds['TinhTrang'] ?></p>
                </div>
                <div style="margin-top: 30px;background-color:white;;margin-right:20px;margin-left:20px;border-radius:5px">
                    <p style="font-weight: bold;"><i class="fas fa-map-marker-alt"></i> Thông tin nhận hàng</p>
                    <div style="display:flex">
                        <div style="width:650px;margin-left:20px">
                            <p>Người nhận: <?php echo $ds['TenNguoiNhan'] ?></p>
                            <p>Địa chỉ nhận hàng: <?php echo $ds['DiaChiNhanHang'] ?></p>
                        </div>
                        <div style="width:650px">
                            <p>Ngày Lập hóa đơn: <?php echo $ds['NgayLap'] ?></p>
                ...
                <div style="margin-top: 30px;background-color:white;;margin-right:20px;margin-left:20px;border-radius:5px">
                    <p style="font-weight: bold;"><i class="fas fa-credit-card"></i> Thông tin sản phẩm</p>
                    <?php
                    $tong = 0;
                    while ($ds = $giatri1->fetch_assoc()) {
                        $ds2 = $giatri2->fetch_assoc();
                    ?>
                        <div style="display:flex;margin-top:20px;margin-left:20px">
                            ...
                            <a style="color: black;" href="ChiTiet.php?id=<?php echo $ds['MaSanPham']?>"><p><?php echo $ds['TenSanPham'] ?></p></a>
                                <p>Số lượng: <?php echo $ds2['SL'] / 3 ?></p>
                            </div>
                            <div style="margin-left:350px">Đơn giá: <?php echo number_format($ds['DonGia']) . ' VNĐ' ?></div>
                        </div>
                        <hr>
                    ...
                    <?php
                        $tong = $tong + ($ds2['SL'] / 3) * $ds['DonGia'];
                        $ds = $giatri1->fetch_assoc();
                        $ds = $giatri1->fetch_assoc();
                    } ?>
                    <div style="margin-left:20px;display:flex">
                        <p style="font-weight: bold;">Tổng tiền:</p>
                        <p style="margin-left:910px"><?php echo number_format($tong) . " VNĐ" ?></p>
                    </div>
